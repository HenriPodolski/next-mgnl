...
WORKDIR /app
COPY package.json package-lock.json packages/web/package.json packages/web/package-lock.json packages/lib/package.json packages/lib/package-lock.json  ./
RUN npm ci
...
RUN npm run build:web && npm ci --production --ignore-scripts --prefer-offline
